    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < articleArray.length(); i++) {
                ...
                int tagsLenght = tagsArray.length();

                if (tagsLenght == 1) {
                    // Create a JSONObject for the author name
                    JSONObject jsonObject = tagsArray.getJSONObject(0);

                    authorName = jsonObject.getString("webTitle");
                }

                    // Create a new {@link News} object with the title, section name, published date,
                    // and url from the JSON response.
                    News article = new News(title, section, authorName, publishedDate, url);
